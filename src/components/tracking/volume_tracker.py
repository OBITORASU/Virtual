...
def controlVolume(
    ...
    """Takes an image stream in the form of an NDArray. It detects hand
    landmarks present in the image stream and calculates the offset length
    between the tip of the thumb and the index finger. Then, it calls
    src.components.helpers.volume_manager.manageVol with the offset length as
    an argument to manipulate the system volume. Optionally returns the image
    stream with drawings if draw is set to True, else it just returns the
    image as is .
    ...
    Args:
        ...
        draw (bool, optional): Decides whether to enable or disable drawing on
